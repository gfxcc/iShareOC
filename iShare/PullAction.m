...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    } else if (-[[change objectForKey:@"new"] CGPointValue].y > self.originalContentInset.top) {
        /* view at top */
    ...
    } else if (_scrollView.contentSize.height < _scrollView.bounds.size.height) {
        /* view at bot and contentSize.height < bounds.size.height */
        _contentView.atBot = YES;
        height = [[change objectForKey:@"new"] CGPointValue].y + self.originalContentInset.top;
        self.frame = CGRectMake(0, _scrollView.bounds.size.height
    ...
    CGRect f = self.frame;
    NSLog(@"t");
    CGPoint rr = self.scrollView.contentOffset;
    if (_refreshing) {
        _lastOffset = offset;
        if (offset != 0) {
            _ignoreInset = YES;
            _ignoreOffset = YES;
            BOOL flag = ((PullActionDefaultContentView*)_contentView).atBot;
            if (!flag) {
                ...
                _currentTopInset = pMaxDistance;
                //[self.scrollView setContentInset:UIEdgeInsetsMake(_currentTopInset + self.originalContentInset.top, self.originalContentInset.left, self.originalContentInset.bottom, self.originalContentInset.right)];
                CGPoint newContentOffset = CGPointMake(0, _scrollView.contentSize.height - _scrollView.bounds.size.height + pMaxDistance);
